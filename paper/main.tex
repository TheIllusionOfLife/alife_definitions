...
\newcommand{\kappaDDci}[1]{%
  \ifthenelse{\equal{#1}{D1D2}}{[0.86,\,0.91]}{%
  \ifthenelse{\equal{#1}{D1D3}}{[0.42,\,0.54]}{%
  \ifthenelse{\equal{#1}{D1D4}}{[0.24,\,0.36]}{%
  \ifthenelse{\equal{#1}{D2D3}}{[0.36,\,0.47]}{%
  \ifthenelse{\equal{#1}{D2D4}}{[0.20,\,0.30]}{%
  \ifthenelse{\equal{#1}{D3D4}}{[0.09,\,0.22]}{??}}}}}}}
\newcommand{\rhoDDci}[1]{%
  \ifthenelse{\equal{#1}{D1D2}}{[0.66,\,0.71]}{%
  \ifthenelse{\equal{#1}{D1D3}}{[0.76,\,0.79]}{%
  \ifthenelse{\equal{#1}{D1D4}}{[0.71,\,0.77]}{%
  \ifthenelse{\equal{#1}{D2D3}}{[0.56,\,0.62]}{%
  \ifthenelse{\equal{#1}{D2D4}}{[0.37,\,0.46]}{%
  \ifthenelse{\equal{#1}{D3D4}}{[0.59,\,0.65]}{??}}}}}}}
\newcommand{\fleissKci}{[0.40,\,0.46]}
\newcommand{\aucDci}[1]{%
...
Primary (confirmatory) results use the default operationalizations defined
above: geometric-mean aggregation for D1, Bonferroni edge combination for
D3, and hash-based genome similarity for D4.  Alternative
operationalizations---arithmetic/harmonic/min aggregation (D1), AND-condition
edge detection (D3), L2-distance genome similarity (D4), and alternative
binarization thresholds (q25/q75)---are reported as \emph{exploratory
robustness checks} and are not used for ranking conclusions.  The primary
prediction target is alive-count AUC; recovery time and lineage diversity
...
reproductive statistics intact.  Specifically, E4's Gaussian noise
($\sigma = 0.5$) on neural inputs degrades resource harvesting
efficiency, reducing the energy--waste coupling signal that D1's
metabolism criterion relies on; D2's reproduction and selection
sub-scores, computed from discrete lineage events, are comparatively
robust to continuous sensory perturbation.
This suggests that definition-level
...
and D2 (\aucD{D2}).  D1's seven criteria capture the broadest signal, while D2's three
components (reproduction, heredity, selection) produce a narrower score
range that limits threshold discrimination.  Alternative targets
partially mitigate substrate bias:
...
$\pm 20\%$ weight perturbation preserves family ordering; exploratory
robustness sweeps over alternative aggregation modes (arithmetic,
harmonic, min for D1), edge combination modes (AND-condition for D3),
and genome similarity metrics (L2-distance for D4) confirm rank stability
across variants (see Table~\ref{tab:threats}).
...
and the Lenia cross-substrate test (\S\ref{sec:lenia}) partially address
this, but testing on fundamentally different substrates
(e.g., chemical reaction networks, wet-lab protocells) is needed.
(3)~\emph{Price equation transmission}: we report only the selection
component of the Price equation; the full transmission--selection
decomposition (requiring multi-generation lineage tracking with
parent--offspring trait regression) is left for future work.
(4)~\emph{Temporal stability}: D3's SCC membership shows moderate
Jaccard stability across time windows, suggesting that organizational
closure is a persistent but not perfectly stationary property; longer
runs may be needed to characterize steady-state closure.
...
(Cohen's $d$ and sign) regardless of outcome; a null result would
indicate that the D2--D3 axis is substrate-specific, which is equally
informative.
...
toolchain version is recorded in \texttt{rust-toolchain.toml}.
Deterministic builds are ensured by \texttt{uv sync --frozen}.
