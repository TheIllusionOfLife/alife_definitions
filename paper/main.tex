...
% --- Result values (from analysis pipeline, seeds 0-99, 5 regimes) ---
\newcommand{\kappaDD}[1]{%
...
D1 and D2 show the highest pairwise agreement
($\kappa = $ \kappaDD{D1D2}, $\rho = $ \rhoDD{D1D2}),
reflecting their shared reliance on reproduction and evolutionary signals.
...
Single-family runs (in progress; F1 complete across all regimes) confirm
that the F1 $>$ F2 $>$ F3 ordering on D1 holds in isolation.
Preliminary results show F3's D3 closure score is comparable without
competition, suggesting that the qualitative disagreement structure
(D2 vs.\ D3) is preserved in both conditions.  Full single-family
results for F2 and F3 will be included in the camera-ready version.
...
and D2 (0.67).  The \emph{lineage diversity} target produced degenerate
results (all ROC-AUC $\approx$ 0.50), suggesting that genome hash
diversity does not provide a useful continuous prediction signal in this
substrate.  Recovery time is the most discriminative target overall.
...
validates the TE-based coupling detection at $n = 200$ sample points.
