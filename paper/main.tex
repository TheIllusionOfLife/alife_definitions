...
% --- Result placeholders (auto-fill from analysis pipeline) ---
\newcommand{\kappaDD}[1]{\textbf{??\ensuremath{\kappa}}}   % Cohen's kappa placeholder
\newcommand{\rhoDD}[1]{\textbf{??\ensuremath{\rho}}}       % Spearman rho placeholder
\newcommand{\fleissK}{\textbf{??}}                          % Fleiss' kappa
\newcommand{\aucD}[1]{\textbf{??}}                          % ROC-AUC per definition
\newcommand{\TODO}[1]{\textbf{[#1]}}                        % generic placeholder
% --- End placeholders ---
...
\emph{lineage diversity} (fraction of unique genome hashes in the
final 20\% of lineage events).
...
D2 and D4 show the highest pairwise agreement
(Spearman $\rho = $ \rhoDD{D2\_D4}, Cohen's $\kappa = $ \kappaDD{D2\_D4}),
reflecting their shared reliance on evolutionary signals.
D2 and D3 show the lowest $\kappa$ (\kappaDD{D2\_D3}),
...
strictness.  The aggregate Fleiss' $\kappa$ across all four definitions
is \fleissK{} (95\% bootstrap CI: \TODO{CI}).
...
axis is preserved: F3 achieves non-zero closure (mean SCC includes
\TODO{N} of 5 process variables) while scoring zero on D2.
...
and F2 (\TODO{values}), confirming that differential reproductive
success correlates with genome distance.  F3, lacking reproduction,
returns zero.  The Price-based $S_\text{select}$ sub-score strengthens
D2's discrimination: F1 and F2 show clear selection signals absent in F3.

\paragraph{Competition controls.}
Single-family runs confirm that the F1 $>$ F2 $>$ F3 ordering on D1
holds in isolation (\TODO{values}).  F3's D3 closure score is
\TODO{higher/comparable} without competition, suggesting that resource
deprivation from F1 partially suppresses F3's self-maintenance in
coexistence runs.  The qualitative disagreement structure (D2 vs.\ D3)
is preserved in both conditions.

\paragraph{Predictive validity.}
Figure~\ref{fig:roc} shows ROC performance on the held-out test set
(Table~\ref{tab:predictive}).
Definitions incorporating evolutionary signals (D2, D4) predict
population robustness more accurately
(ROC-AUC: D2 = \aucD{D2}, D4 = \aucD{D4}).
D3 shows lower predictive validity
(ROC-AUC = \aucD{D3}) because organizational closure without reproduction
cannot sustain populations indefinitely under E2 or E3.
...
Balanced Acc.\    & \TODO{} & \TODO{} & \TODO{} & \TODO{} \\
...
Under alternative targets, definition rankings shift:
with \emph{recovery time}, D3 performs comparably to D2 (\TODO{values}),
as self-maintaining families recover without reproduction.
With \emph{lineage diversity}, D4 outperforms D2 (\TODO{values}),
consistent with D4's focus on information preservation.
...
\TODO{$\Delta$} but preserves the F1 $>$ F2 $>$ F3 ordering across all
...
edges from \TODO{N} to \TODO{N} and mean closure from \TODO{val} to
\TODO{val}, but the qualitative agreement structure is preserved.

\paragraph{Surrogate false positive rate.}
Phase-randomized surrogate analysis confirms that TE-based coupling
detection maintains a false positive rate below \TODO{val}\% across all
adapters (100 surrogates per pair), validating the statistical procedure
at $n = 200$ sample points.
...
D2 and D4 predict population robustness (alive-count AUC) better than
D1 and D3, but this partly reflects the substrate's design: reproduction
is the dominant recovery mechanism.  Alternative targets partially
mitigate this bias: under recovery time, D3 closes the gap with D2,
and under lineage diversity, D4 outperforms D2.
