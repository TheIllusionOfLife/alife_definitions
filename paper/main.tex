...
differences.  The D2--D3 disagreement axis is preserved in both conditions.
...
preserving family ordering.  D3 FDR sweep ($q$: 0.01--0.10) increases
mean closure from 0.48 to 0.61 but preserves the qualitative agreement
structure.
...
exceeds 0.11.  Only the source series is randomized, preserving the
target's autocorrelation to test directional TE specifically.
...
while D2's three components produce a narrower score range limiting
threshold discrimination (AUC = \aucD{D2}).
...
(2)~\emph{Substrate bias}: explicit reproduction mechanisms may favor
evolution-centric definitions; alternative targets and the Lenia test
(\S\ref{sec:lenia}) partially address this, but fundamentally different
substrates (chemical networks, protocells) are needed.
...
A single script (\texttt{reproduce\_all.sh}) runs the full pipeline:
benchmark generation (${\sim}$30\,h on a Mac Mini M2~Pro),
scoring, analysis, and figure generation.
All Python dependencies are pinned via \texttt{uv.lock}; the Rust
toolchain is recorded in \texttt{rust-toolchain.toml}.
