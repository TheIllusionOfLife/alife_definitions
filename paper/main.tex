...
Transfer entropy is estimated with 5 equal-width bins on
$z$-scored series ($\mu = 0$, $\sigma = 1$) at lag~1; significance is
assessed by 400 time-shifted permutations.  Granger causality uses
OLS regression at lags 1--5, selecting the lag with the lowest
...
decomposition provides a \emph{direct} fitness decomposition from
lineage records, avoiding proxy-based correlation measures that could
conflate selection with drift.
...
Definition-level sensitivity to perturbation is itself an informative
diagnostic.

\paragraph{Competition vs.\ capability.}
Single-family controls confirm that the disagreement structure persists
without competition.  F3's D3 closure improves modestly in isolation,
but the D2--D3 divergence axis is preserved, confirming that definition
...
D2's three components produce a narrower score range limiting
threshold discrimination.
Under lineage diversity, all definitions converge to a narrow
AUC band (0.77--0.83), confirming complementary information capture.
Predictive validity should not be interpreted as a ranking of
``correctness''---it is substrate- and target-dependent.
...
the full transmission--selection decomposition is left for future work.
...
We state a pre-registered hypothesis:
...
Lenia creatures exhibit self-maintaining spatial organization
(captured by D3's closure score) but lack reproduction or heritable
variation (required by D2), confirming that the D2--D3 disagreement
axis reflects a genuine conceptual divide rather than a substrate artifact.
...
\texttt{reproduce\_all.sh} runs the full pipeline
(${\sim}$30\,h on a Mac Mini M2~Pro).
Dependencies are pinned via \texttt{uv.lock} and \texttt{rust-toolchain.toml}.
Raw benchmark JSONs (${\sim}$10\,GB) are archived on Zenodo;
the score matrix TSV and analysis JSONs are included in the repository.
